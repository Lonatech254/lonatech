...
async function auth(req, res, next) {
  try {
    ...
    const redirectToAuth = () => {
      ...
      return res.redirect(`/order/auth${query}`);
    };
    ...
    // Check if token was invalidated (e.g., user logged out)
    const isInvalid = await userInvalidTokens.findOne({ accessToken: token });
    if (isInvalid) {
      return redirectToAuth();
    }
    ...
    try {
      ...
      if (err instanceof jwt.TokenExpiredError) {
        ...
        try {
          const decodedRefresh = jwt.verify(refreshToken, config.refreshTokenSecret);
          const newDecoded = await handleTokenRefresh(req, res, decodedRefresh);
      ...
      throw err; // Forward unknown error
    }
  } catch (err) {
